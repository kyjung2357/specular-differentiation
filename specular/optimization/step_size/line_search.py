...
class LineSearch:
    ...
    def __init__(
        ...
        *,
        ...
        t_k: Scalar | None = None,
        t_max: Scalar | None = None,
    ):
        """
        Parameters:
            name: Line search rule name (see ``_SUPPORTED_OPTIONS``).
            f: Objective function. May instead be supplied when calling the rule.
            h: Mesh size for finite difference gradient approximation.
            zero_tol: Tolerance for specular gradients.
            t_0: Initial trial step length. ``t_k`` is accepted as an alias.
            ...
            max_alpha: Maximum trial step length. ``t_max`` is accepted as an alias.
        ...
        if self._base_name == "Wolfe":
            if f is None:
                raise ValueError(
                    f"Line search '{name}' requires objective function 'f' to compute gradients."
                )
            if not (0.0 < c_2 < 1.0):
        ...
        self.f = f
        self.t_k = float(t_0)
        self.t_max = float(max_alpha)
        self.t_0 = self.t_k
        self.max_alpha = self.t_max
        self.h = float(h)
        self.c_1 = float(c_1)
        self.c_2 = float(c_2)
        self.c_3 = float(c_3)
    ...
    def __call__(
        ...
        *,
        ...
        Parameters:
            ...
            f: Objective function, overriding the constructor value when supplied.
            gradient_f: Optional gradient function for Wolfe searches.
        ...
        if objective is None:
            raise ValueError(f"Line search '{self.name}' requires objective function 'f'.")
        ...
        def f_vec(z: np.ndarray) -> float:
            if scalar_input:
                ...
                return float(objective_scalar(float(z.ravel()[0])))
            objective_vector = cast(VectorToScalarFunc, objective)
            return float(objective_vector(z))
        ...
        def grad_vec(z: np.ndarray) -> np.ndarray:
            ...
            if self.name.startswith("specular_"):
                if scalar_input:
                    objective_scalar = cast(ScalarToScalarFunc, objective)
                    value = derivative(
                        objective_scalar,
            ...
            for i in range(z.size):
                ...
                step[i] = self.h
                result[i] = (f_vec(z + step) - f_vec(z - step)) / (2.0 * self.h)
            return result
