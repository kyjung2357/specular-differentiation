...
class StepSchedule:
    ...
    def __init__(
        self,
        name: str,
        parameters: float | int | Sequence[float] | Callable | None = None,
        *,
        ...
        if parameters is not None:
            if any(value is not None for value in (a, b, r, user_defined_rule)):
                raise TypeError(
                    "Use either the legacy 'parameters' argument or named rule parameters, not both."
                )
            if name in {"constant", "not_summable"}:
                a = parameters  # type: ignore[assignment]
            elif name == "square_summable_not_summable":
                try:
                    a, b = parameters  # type: ignore[misc]
                except (TypeError, ValueError):
                    raise ValueError(
                        "Rule 'square_summable_not_summable' requires parameters [a, b]."
                    ) from None
            elif name == "geometric_series":
                try:
                    a, r = parameters  # type: ignore[misc]
                except (TypeError, ValueError):
                    raise ValueError(
                        "Rule 'geometric_series' requires parameters [a, r]."
                    ) from None
            elif name == "user_defined":
                user_defined_rule = parameters  # type: ignore[assignment]
