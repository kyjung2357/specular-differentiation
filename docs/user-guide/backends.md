...
NumPy is installed with `specular-differentiation`. Optional backends require
...
The package requires Python 3.14 or newer. Select an optional backend only after
its dependencies have been installed. Selection imports the backend immediately
and reports a missing dependency before calculations begin.

For individual signatures and arguments, see
[`specular.backends`](../api/backends/index.md).
...
The setting is isolated between asynchronous contexts. A fresh execution
context starts with NumPy; a thread that explicitly inherits a context
inherits that context's backend selection.
...
The JAX backend samples coordinates in bounded batches rather than building
a dense `n` by `n` identity matrix, which limits temporary storage for large
gradients. A Jacobian still requires storage for its returned `m` by `n` matrix.
