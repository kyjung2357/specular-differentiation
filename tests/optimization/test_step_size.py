...
def test_constant_step_size():
    ...
    # a = 0.5
    step = StepSchedule(name='constant', parameters=0.5)
...
def test_not_summable():
    ...
    # a = 2.0
    step = StepSchedule(name='not_summable', parameters=2.0)
...
def test_geometric_series():
    ...
    # a = 1.0, r = 0.5
    step = StepSchedule(name='geometric_series', parameters=[1.0, 0.5])
...
def test_user_defined():
    ...
    step = StepSchedule(name='user_defined', parameters=custom_rule)
...
def test_square_summable_error():
    with pytest.raises(ValueError, match="Invalid length"):
        StepSchedule('square_summable_not_summable', [1.0])

    # a <= 0
    with pytest.raises(ValueError):
        StepSchedule('square_summable_not_summable', [-1.0, 1.0])

    # b < 0
    with pytest.raises(ValueError):
        StepSchedule('square_summable_not_summable', [1.0, -0.1])

def test_geometric_error():
    # r >= 1
    with pytest.raises(ValueError):
        StepSchedule('geometric_series', [1.0, 1.1])
    
    # r <= 0
    with pytest.raises(ValueError):
        StepSchedule('geometric_series', [1.0, -0.5])

def test_user_defined_error():
    with pytest.raises(TypeError):
        StepSchedule('user_defined', "not a function") # type: ignore