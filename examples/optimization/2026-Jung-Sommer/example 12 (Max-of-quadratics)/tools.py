...
def report_results(
    ...
    for name, results_list in all_results.items():
        ...
        plot_curve = mean_curve.clip(lower=1e-16)

        plt.plot(x_data, plot_curve, label=name, color=color, linewidth=1.5)
        # plt.plot(x_data, df.median(axis=1).clip(lower=1e-16), color=color, linestyle="--", alpha=0.5, linewidth=1)

    print("\n[Running Time Summary]")
    for name, times in running_times.items():
        if times:
            avg_time = sum(times) / len(times)
            print(f"{name:9s}: {avg_time:.5f} sec")
    ...
    path_txt = os.path.join(base_dir, f"tables/table{file_number}-{suffix}.txt")
    path_fig = os.path.join(base_dir, f"figures/figure{file_number}-{suffix}.{'pdf' if pdf else 'png'}")
    ...
    plt.title(problem_label, fontsize=10)
    plt.grid(True)
    plt.xscale("log")
    plt.yscale("log")
