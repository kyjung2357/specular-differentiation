...
def run_single_trial(args):
    ...
    centers = rng.standard_normal((components, dimension))
    centers /= np.maximum(np.linalg.norm(centers, axis=1, keepdims=True), 1e-12)
    centers *= radius
    diagonal_weights = rng.uniform(0.5, 2.0, size=(components, dimension))
    offsets = 0.05 * rng.random(components)
    x_0 = rng.normal(scale=radius, size=dimension)

    centers_torch = torch.tensor(centers, dtype=torch.float32)
    weights_torch = torch.tensor(diagonal_weights, dtype=torch.float32)
    offsets_torch = torch.tensor(offsets, dtype=torch.float32)

    def f(x):
        x = np.asarray(x, dtype=float).reshape(-1)
        diff = x[None, :] - centers
        values = 0.5 * np.sum(diagonal_weights * diff**2, axis=1) + offsets
        return float(np.max(values) + 0.5 * lambda2 * np.dot(x, x))

    def f_torch(x_tensor):
        diff = x_tensor[None, :] - centers_torch
        values = 0.5 * torch.sum(weights_torch * diff**2, dim=1) + offsets_torch
        return torch.max(values) + 0.5 * lambda2 * torch.sum(x_tensor**2)
...
def run_experiment(
    ...
    print("Problem: max-of-quadratics")
    print(f"Settings: components={components}, dimension={dimension}, radius={radius}, lambda_2={lambda2}")
    active_bfgs_rules = {
    ...
    tasks = [
        (i, components, dimension, radius, lambda2, iteration, methods, trials)
    ...
    suffix = f"{components}-{dimension}-{radius}-{lambda2}"
    report_results(
        all_results,
        running_times,
        file_number,
